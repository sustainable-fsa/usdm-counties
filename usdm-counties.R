...
usdm_counties %>%
  arrow::write_parquet(sink = "usdm-counties.parquet",
                       version = "latest",
                       compression = "zstd",
                       use_dictionary = TRUE)
...
##   const d = await (await fetch('usdm-counties.json')).json();
...
stopifnot(identical(levels(usdm_counties$usdm_class), web_classes))
web_max <-
  usdm_counties %>%
  dplyr::transmute(
...
## ... then one guarded undo of the double latin1-to-UTF-8 encoding from the
## county read above: reinterpret the string's latin1 bytes as UTF-8, keep
## the result only when it is valid and different — "DoÃ±a Ana" heals,
## "Doña Ana" and ASCII survive. The archive itself is untouched.
repair_mojibake <- function(x){
  y <- iconv(x, from = "UTF-8", to = "latin1")
  Encoding(y) <- "UTF-8"
  dplyr::if_else(!is.na(y) & validUTF8(y) & y != x, y, x)
}
web_names <- dplyr::mutate(web_names, County = repair_mojibake(County))
stopifnot(!any(grepl("\u00c3", web_names$County)))
...
jsonlite::write_json(
  list(
    ...
    dataset = jsonlite::unbox("usdm-counties"),
...
s3_put(s3_bucket_name, paste0(s3_prefix, "/usdm-counties.json"),
       "usdm-counties.json",
...
cf_invalidate(c(paste0("/", s3_prefix, "/usdm-counties.parquet"),
                paste0("/", s3_prefix, "/usdm-counties.json"),
...
cf_wait_manifest("https://data.sustainable-fsa.com/usdm-counties/manifest.json",
                 "manifest.json")
